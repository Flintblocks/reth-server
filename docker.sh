...
do
  ...
  echo "docker run -d --name reth-http-$i -p $port_http:8545 -v ~/chain/reth/data/db:/data/db -e RETH_DB_PATH=/data/db reth-server-reth-http"
...
do
  ...
  echo "docker run -d --name reth-ws-$i -p $port_ws:8546 -v ~/chain/reth/data/db:/data/db -e RETH_DB_PATH=/data/db reth-server-reth-ws"
