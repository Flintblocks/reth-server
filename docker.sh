...
do
  ...
  docker run -d --name reth-server-reth-http-$i -p $port_http:8545 -v ~/chain/reth/data/db:/data/db -e RETH_DB_PATH=/data/db reth-http
done
...
do
  ...
  docker run -d --name reth-server-reth-ws-$i -p $port_ws:8546 -v ~/chain/reth/data/db:/data/db -e RETH_DB_PATH=/data/db reth-ws
done
